...
//Read initial configuration
  ...
  for (int i=0; i<npart; ++i)
  {
    ReadConf >> x[i] >> y[i] >> z[i];       //read configuration
    x[i] = Pbc( x[i] * box );
    y[i] = Pbc( y[i] * box );
    z[i] = Pbc( z[i] * box );
  }
...
//Print initial values for measured properties
  ...
  cout << "Initial total energy     = " << walker[ie] << endl;
  cout << "Initial pressure         = " << ( walker[ipr] + rho*temp ) << endl;  
...
// distance i-j in pbc
  ...
  walker[ie] = (4.0 * v + kin) /(double)npart;  // Total energy;
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ipr] = rho * walker[it] + ( 16. * p + ptail * (double)npart * 3 ) / vol;  //Pressure with tail correction

  //############## For exercise 07.2, print istantaneous value of U/N  ################### comment if not needed

  // ofstream Instant("output_instant_gas.out", ios::app);
  // const int wd=12;

  // Instant << setw(wd) << walker[iv] << endl;
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pres, Gdir, GdirFinal;   //aggiunto pressione
   const int wd=12;
    ...
    Gdir.open("output_gdir_prog.dat",ios::app);
    GdirFinal.open("output_gdir.dat",ios::app);
    

    stima_pot = blk_av[iv]/blk_norm; // + vtail; //Potential energy with tail correction
    ...
    stima_pres = blk_av[ipr]/blk_norm;  //Pressure with tail correction
    ...
    for(int i=0; i < nbins; i++){                //update g(r) bins, print progressive values
      ...
      err_gdir=Error(glob_av[5+i],glob_av2[5+i],iblk);

      Gdir << setw(wd) << iblk <<  setw(wd) << stima_gdir << setw(wd) << glob_av[5+i]/(double)iblk << setw(wd) << err_gdir << endl;
    }
    Gdir << endl << endl;
    ...
    if(iblk==nblk){       //Print last g(r) on separate file
      ...
      for(int i=0; i < nbins; i++){
        ...
        GdirFinal << setw(18) << (double) i * bin_size << setw(18) << glob_av[5+i]/(double)nblk << setw(18) << err_gdir << endl;
      }
    }
...
void ConfFinal(void)
{
  ofstream WriteConf, WriteVelocity, WriteSeed, Gdir;
